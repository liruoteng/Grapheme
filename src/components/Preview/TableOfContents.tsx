...
function parseHeadings(content: string, isMarkdown: boolean): TocEntry[] {
  const entries: TocEntry[] = [];
  const lines = content.split("\n");
  const re = isMarkdown ? /^(#+)\s+(.+)/ : /^(=+)\s+(.+)/;
...
export function TableOfContents() {
  ...
  const entries = useMemo(
    () => (activeTab ? parseHeadings(activeTab.content, isMd) : []),
    [activeTab?.content, isMd] // eslint-disable-line react-hooks/exhaustive-deps
  );

  if (!activeTab || !(activeTab.path.endsWith(".typ") || isMd)) {
    return (
      <div className="toc-panel toc-empty">
        <p>No .typ or .md file open</p>
      </div>
    );
  }
